-- =====================================================================
-- FIX FARM ANALYTICS - ABSOLUTELY NO MULTIPLICATION POSSIBLE
-- =====================================================================
-- Strategy: Pre-calculate everything in CTEs with DISTINCT ON
-- Then just SUM the pre-calculated values
-- =====================================================================
...
SELECT
    ...
    COALESCE(COUNT(DISTINCT fa.unique_products), 0)::bigint AS unique_products,
...
FROM farms f
LEFT JOIN farm_aggregates fa ON f.id = fa.farm_id
GROUP BY f.id, f.client_id, f.name, f.code
ORDER BY total_value_allocated DESC NULLS LAST;

COMMENT ON VIEW public.vw_allocation_analytics_by_farm IS 'Farm analytics with guaranteed no multiplication using DISTINCT ON';
